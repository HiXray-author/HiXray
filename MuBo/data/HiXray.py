...
HiXray_CLASSES = (
    'Portable_Charger_1', 'Portable_Charger_2','Water','Laptop','Mobile_Phone','Tablet','Cosmetic','Nonmetallic_Lighter',
)

HiXray_ROOT = "/mnt/cvpr_dataset/test/"

class HiXrayAnnotationTransform(object):
    ...
    def __init__(self, class_to_ind=None, keep_difficult=False):
        self.class_to_ind = class_to_ind or dict(
            zip(HiXray_CLASSES, range(len(HiXray_CLASSES))))
...
class HiXrayDetection(data.Dataset):
    def __init__(self, root,
                 image_sets,
                 transform=None, target_transform=HiXrayAnnotationTransform(),phase=None):
        #self.root = root
        self.root = HiXray_ROOT
        self.image_set = image_sets
        self.transform = transform
        self.target_transform = target_transform
        #self.name = dataset_name
        self.name = 'HiXray0723_knife'
        if(phase == 'test'):
